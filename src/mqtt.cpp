...
namespace Mqtt
{
const bool controlPacketRequiresIdentifier(uint8_t control_packet_type, uint8_t qos)
{
	return (control_packet_type == ControlPacketType::PUBLISH && qos > 0) ||
	       control_packet_type == ControlPacketType::SUBSCRIBE || control_packet_type == ControlPacketType::UNSUBSCRIBE ||
	       control_packet_type == ControlPacketType::PUBACK || control_packet_type == ControlPacketType::PUBREC ||
	       control_packet_type == ControlPacketType::PUBREL || control_packet_type == ControlPacketType::PUBCOMP;
}
...
Publish publishFromBytes(uint8_t header, int32_t len, uint8_t *buffer)
{
	...
	if (buffer_iter[0] == 0) {
		...
		cout << "Unhandled Properties Length: " << properties << " ? " << properties_len << endl;
	}

	const string message = string((char *)buffer_iter, len - (buffer_iter - buffer));
...
void connect(boost::asio::ip::tcp::socket &socket, string client_id, string username, string password)
{
	uint8_t fixed_header = ControlPacketType::CONNECT << 4;

	size_t packet_len = client_id.length() + 2 + username.length() + 2 + password.length() + 2 + 11;
	uint8_t encoded_control_packet_length[4];
	uint8_t bytes_for_packet_length = encodeInt(encoded_control_packet_length, packet_len);
	...
	control_packet_iter[7] = ConnectFlags::USERNAME | ConnectFlags::PASSWORD | ConnectFlags::CLEAN_SESSION;
...
void publish(boost::asio::ip::tcp::socket &socket, string topic, string message, uint8_t qos, bool retain,
	     uint16_t packet_identifier = 1)
{
	...
	size_t packet_len = 1 + (topic.length() + 2) + (qos > 0 ? 2 : 0) + message.length();
	uint8_t encoded_control_packet_length[4];
	uint8_t bytes_for_packet_length = encodeInt(encoded_control_packet_length, packet_len);
...
void subscribe(boost::asio::ip::tcp::socket &socket, string topic, uint8_t qos, uint16_t packet_identifier)
{
...
{
	return control_packet_type == ControlPacketType::CONNECT || control_packet_type == ControlPacketType::CONNACK ||
	       control_packet_type == ControlPacketType::PUBLISH || control_packet_type == ControlPacketType::PUBACK ||
	       control_packet_type == ControlPacketType::PUBREC || control_packet_type == ControlPacketType::PUBREL ||
	       control_packet_type == ControlPacketType::PUBCOMP || control_packet_type == ControlPacketType::SUBSCRIBE ||
	       control_packet_type == ControlPacketType::SUBACK || control_packet_type == ControlPacketType::UNSUBSCRIBE ||
	       control_packet_type == ControlPacketType::UNSUBACK || control_packet_type == ControlPacketType::PINGREQ ||
	       control_packet_type == ControlPacketType::PINGRESP || control_packet_type == ControlPacketType::DISCONNECT ||
...
void Session::handleSocket()
{
	...
	if (isValidCommandType(command)) {
		...
		if (command == ControlPacketType::CONNACK) {
			...
			cout << "Session Expiry Interval: " << (int)connack.session_expiry_interval
			     << " Receive Maximum: " << (int)connack.receive_maximum << " Maximum QoS: " << (int)connack.maximum_qos
			     << " Retain Available: " << connack.retain_available
			     << " Maximum Packet Size: " << (int)connack.maximum_packet_size
			     << " Assigned Client Identifier: " << connack.assigned_client_identifier
			     << " Topic Alias Maximum: " << (int)connack.topic_alias_maximum
			     << " Reason String: " << connack.reason_string << " User Property: " << connack.user_property
			     << " Wildcard Subscription Available: " << connack.wildcard_subscription_available
			     << " Subscription Identifier Available: " << connack.subscription_identifier_available
			     << " Shared Subscription Available: " << connack.shared_subscription_available << endl;
			isConnected = true;
			...
			if (debug) {
				...
				cout << "QoS: " << (int)publish.qos << " Retain: " << publish.retain
				     << " Packet Identifier: " << publish.packet_identifier << " Topic: " << publish.topic
				     << " Message: " << publish.message << endl;
			}
...
void Session::init(string addr, int port, string client_id, string username, string password)
{
